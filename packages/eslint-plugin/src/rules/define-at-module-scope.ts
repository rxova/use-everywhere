 ...
 * first registration, so a second `defineStore('settings', { persist })` after
 ...
 */
const DEFINERS = new Set(['defineStore', 'defineChannel', 'createNamespace']);

export const defineAtModuleScope: Rule.RuleModule = {
  meta: {
    type: 'problem',
    docs: {
      description: 'Require defineStore, defineChannel and createNamespace at module scope',
      recommended: true,
